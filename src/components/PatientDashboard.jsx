          ...
          (async () => {
            try {
              ...
              if (dept) {
                ...
                // Fetch room assignment for this department
                const roomQ = query(collection(db, "doctor_rooms"), where("department", "==", dept));
                const roomSnap = await getDocs(roomQ);
                if (!roomSnap.empty) {
                  setAssignedRoom(roomSnap.docs[0].data());
                } else {
            ...
            {hasActiveQueue ? (
              <>
                <div className="pd-doctor-line">
                  ...
                  <div className="doc-text">You will be seen by: <strong style={{ marginLeft: 6 }}>{assignedRoom?.doctorName || "Dr. Rao"}</strong></div>
                </div>
                {assignedRoom && (
                  <div className="pd-doctor-line" style={{ marginTop: 8 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke="#10b981" strokeWidth="2"/></svg>
                    <div className="doc-text">Room: <strong style={{ marginLeft: 6 }}>{assignedRoom.roomNumber}</strong> ({assignedRoom.department})</div>
                  </div>
                )}
              </>
            ) : (
